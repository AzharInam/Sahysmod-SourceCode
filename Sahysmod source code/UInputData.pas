...
uses Dialogs, SysUtils, UdataMod, UMainForm, UExtraUtils;
...
procedure ReadInputs (var SelfDone : boolean);
var FileToOpen, Reading, YrStr, PrevStr : string;
    i, j, k : integer;
    NrOfParam : byte;
    OK : boolean;
...
begin
  ...
  begin
    YrStr:=inttostr(YearNr);
    FileToOpen := AnnualFileName;
    FileToOpen := ChangeFileExt(FileToOpen,'');
    if YearNr>2 then
    begin
      PrevStr:=inttostr(YearNr-1);
      while Pos(PrevStr,FileToOpen) > 0 do
            delete(FileToOpen,Pos(PrevStr,FileToOpen),1);
    end;
    FileToOpen:=FileToOpen+YrStr;
    FileToOpen := ChangeFileExt(FileToOpen,'.inp');
    if not fileexists (FileToOpen) then
    begin
       FileToOpen:=OriginalName;
       if OriginalName='' then FileToOpen:=OpenFileName;
    end;
  end;
  ...
  readln (InputFile);
  SaveOverallGroup;
  for k:=1 to 8 do  setlength (Neighbor[k],NrOfIntPoly+1);
  for k:=1 to 6 do
  begin
    setlength (Conduct[k],NrOfIntPoly+1);
    setlength (TopCond[k],NrOfIntPoly+1);
  end;
  ...
  PrevStr:=extractfilename (FileToOpen);
  showmessage ('The file '+PrevStr+' is opened.');
