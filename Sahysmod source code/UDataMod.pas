...
//NrOfSides is used in groupfiles in UExtraUtils
    ...
    VertCond, TopLayer,
    OrigInflowValue, TopLevel    : DataSetR;
    Neighbor, AuxInt             : DataSetI8;
...
//NrOfNeighbors is same as NrOfSides but used in main program
    ...
    DumStr                       : string[10];
    MainStr                      : array [1..8] of string[12];
    AnnualCalc, EndOfAnnual,
    RemovalStage, RemovalError,
    SeasonalStorEffOK, PolyData, ColorMap,
...
begin
  ...
  for j:=1 to 8 do setlength(Neighbor[j],NrOfPoly+1);
  for j:=1 to 6 do
  begin
    setlength(Conduct[j],NrOfPoly+1);
    setlength(TopCond[j],NrOfPoly+1);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      setlength (TopCond[j],NrOfIntPoly+1);
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        read (GroupFile,HlpValue[j,k]);
        if GroupName='Name11' then
           if (j<3) and (DrainIndex[k]=1) and (HlpValue[j,k]=0) then
               HlpValue[j,k]:=-1;
      end;
      readln (GroupFile);
    end;
  ...
  begin
    setlength (TopLevel,InitNrOfIntPoly);
    setlength (AquiferType,InitNrOfIntPoly);
    for k:=0 to InitNrOfIntPoly-1 do TopLevel[k]:=round(HlpValue[1,k]);
...
begin
  ...
  for k:=0 to InitNrOfIntPoly-1 do TopLevel[k]:=HlpValue[1,k];
  setlength (AquiferType,NrOfIntPoly);
  for k:=0 to InitNrOfIntPoly-1 do AquiferType[k]:=round(HlpValue[4,k]);
...
begin
  ...
  ReadPolyGroup('Name08',4);                             {Agric. practices}
  AssignFile(GroupFile,'Name08');
  {$I-} Rewrite(GroupFile); {$I+}
  if ioresult <> 0 then
  begin
  ...
  begin
    ...
    begin
      ...
      setlength (RotationType,Count);
      RotationType[Count-1]:=round(HlpValue[3,Count-1]);
      setlength (InclIndex,Count);
      InclIndex[Count-1]:=round(HlpValue[4,Count-1]);
    end;
  end;
  closefile (GroupFile);
...
var TmpName:string;
    ...
    Dummy : real;
begin
  TmpName:=OutputFileName;
  if OutputFilename='' then TmpName:=OpenFileName;
  if OutputFileName<>'' then TmpName:=ChangeFileExt(OutputFileName,'.inp');
  ...
  setlength (Xvalue,1);
  Xvalue[0]:=Dummy;
  for i:=1 to 8 do setlength (Neighbor[i],NrOfIntPoly+1);
...
begin
  ...
  for k:=1 to 8 do setlength (Variable[k],NrOfData);
  i:=-1;
    ...
    begin
      ...
      begin
        ...
        readln (OutFile,HlpStr,MainStr[5],HlpStr,MainStr[6],
                        HlpStr,MainStr[7],HlpStr,MainStr[8]);
      end;
      if Signal='Gti' then
      begin
      ...
      begin
        readln (OutFile,HlpStr,MainStr[1],HlpStr,MainStr[2],
                        HlpStr,MainStr[3],HlpStr,MainStr[4]);
        readln (OutFile,HlpStr,MainStr[5],
                        HlpStr,MainStr[6],HlpStr,MainStr[7]);
      end;
      ...
      begin
        ...
        if (MainStr[k]='-') or (MainStr[k]='') or (MainStr[k]='n.a') then
...
2:NrOfItems:=8;
  ...
  begin
    ...
    if Signal='AvC' then
      readln (OutFile,HlpStr,MainStr[1],HlpStr,MainStr[2],HlpStr,MainStr[3]);
    if Signal='CrA' then
    begin
      readln (OutFile,HlpStr,MainStr[1],HlpStr,MainStr[2],
                      HlpStr,MainStr[3],HlpStr,MainStr[4]);
...
begin
  NrOfItems:=7;
  Detect := 'Uc';
  if Signal='CrA' then NrOfItems:=8;
  if Signal='AvC'then
  begin
    NrOfItems:=3;
    Detect:='A';
...
begin
  ...
  NrOfitems := 7;
  if GroupMark='AvC' then NrOfitems := 3;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if GroupMark='EaU' then setlength (Variable[7],SerialNr+1);
          ...
          begin
            ...
            if GroupMark='EaU' then
               Variable[7,SerialNr]:=Variable[1,SerialNr];
          end;
