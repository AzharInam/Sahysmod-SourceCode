...
Type
  ...
  TPolyGroupEdits = array [1..8] of array of Tedit;
  ...
  TMainForm = class(TBaseForm)
    ...
    PolyGroupPanelCancel_Button: TButton;
    TransButton: TButton;
    ...
    PolyGroup_ScrollBox: TScrollBox;
    ...
    procedure NetworkEditExit (Sender: TObject);
    procedure NodesEditExit (Sender: TObject);
    procedure BottomEditExit (Sender: TObject);
    procedure PolyGroupSave_ButtonClick(Sender: TObject);
    procedure PolyGroupPanelCancel_ButtonClick(Sender: TObject);
    ...
    procedure Time_RadioButtonClick(Sender: TObject);
    procedure Poly_RadioButtonClick(Sender: TObject);
  ...
  private
    ...
    procedure SetPolyScrollBoxStandards (const Task : string);
...
uses UDataMod, UCloseForm, UExtraUtils, USymbolsForm, UColumnsForm,
     ...
     UWarningForm, UDataTest, UInitialCalc, UMainCalc, UNodalHelp, UTransForm;
...
//  DecimalSeparator := '.';
  ...
  PolyGroup_ScrollBox.Visible :=false;
  OutputOpened := false;
  GroupOpened := false;
  InputOpened := false;
  GraphOpened := false;
...
begin
  ...
  NetworkForm.Show;
end; {TMainForm.Network_ButtonClick}
{----------------------------------}
...
begin
  ...
  OutputSelection_Button.Visible:=false;
  OutputSymbols_Button.Visible:=false;
  SaveGroup_Button.Visible:=false;
  SeeGraph_Button.Visible:=false;
  GotoInput_Button.Visible:=false;
...
//This occurs when coming in from another tabsheet
      begin
...
begin
  inherited;
    ...
    begin
       Showmessage ('The number of seasons was increased. The season'+
...
//makes strings of general data ('Name0' file) for GeneralInput_Tabsheet
  ...
  InputSave_Button.Caption:='Save all/calculate';
  StatusText('Enter data, then use "Save data" and proceed to other'+
             ' data tabsheets, or use "Save/calculate".');
  GeneralExplanation2_Edit.Text:='Input data values are shown.'+
                                 ' Move mouse over edit boxes for hints.';
  InputOpened := true;
  PolygonalInput_Tabsheet.TabVisible:=true;
  SeasonalInput_Tabsheet.TabVisible:=true;
...
begin
  ...
  begin
    ...
    //Somewhere the NrOfSeasons added was already incorporated in NrOfSeasons
        DataMod.SeasonDuration[k]:= StrToFloat (SeasonDurationEdit[k].Text);
  end;
  CheckGeneralData (DataOK);
  if DataOk then
  begin
...
// between NrOfSeasons and InitNrOfSeasons is made again.
  ...
  begin
    Critical:=true;
    DataMod.NrOfSeasons:=DataMod.InitNrOfSeasons;
    SeasonsAdded:=false;
...
begin
  ...
  begin
    ...
    begin
      ...
      showmessage ('There are one or more input errors, calculations could'+
...
//Messages
    ...
    begin
      ...
      if YesOK then
      begin
        Showmessage ('Go to the input menu to continue. It is not'+
    ...
    if AnnualCalc and (YearNr=NrOfYears) then
    begin
      Showmessage ('The end of the time series is reached. The results'+
      ' can be seen in  '+OrigStr);
      EndOfAnnual := true;
    end;
    chdir (InitDir);
    ...
    if not AnnualCalc then
       if RuntimeError or AccErr then
          showmessage ('An runtime error message will be displayed.')
      else
      begin
         showmessage ('The calculations are completed.'+
         ' The results can be seen in the output file: '+OrigStr);
         if Warning then
         begin
...
begin
  ...
  Critical:=true;
  if SetToGroupSelection then SetPolyComponents('ToGroupEditing');
  DoItSelf:=false;
...
procedure TMainForm.TransButtonClick(Sender: TObject);
{---------------------------------------------------}
begin
  inherited;
...
begin
  ...
  PolyGroup_ScrollBox.Enabled:=true;
  ...
  begin
    ...
    PolyComment_Memo.Lines.Clear;
    PolyComment_Memo.Lines.Add (' " - " = n.a.'         );
    PolyComment_Memo.Lines.Add (' The Krf index is'     );
    PolyComment_Memo.Lines.Add (' only applicable'      );
    PolyComment_Memo.Lines.Add (' when the Kf index'    );
    PolyComment_Memo.Lines.Add (' of farmers responses' );
    PolyComment_Memo.Lines.Add (' is set equal to 1'    );
    PolyComment_Memo.Lines.Add (' (1= Yes)'             );
...
begin
  ...
  if not Destroyed then SetPolyScrollBoxStandards ('Off');
  ...
  begin
    ...
    begin
      if Identity='Overall' then ShowOverallTable;
      exit;
    end else
    begin
...
//This excludes 'Guided'
    ...
    begin
      ...
      begin
        ...
        if not Destroyed then SetPolyScrollBoxStandards ('Off');
        Identity:='Overall';
        if not Destroyed then SetPolyScrollBoxStandards ('On');
        PolyGroup_ButtonClick(Sender);
        exit;
      end;
      NetworkSaved:=true;
...
//        exit;
        DataMod.SaveOverallGroup;
        OverallSaved:=true;
        Showmessage ('The overall data group data was saved despite errors.'+
                     ' Check the network map. Depending on the degree of'+
                     ' error, the program may not function well.');
        Critical:=false;
      end;
      if not Destroyed then SetPolyScrollBoxStandards ('Off');
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          Showmessage ('The agricultural practices group data were checked'+
              ' and saved.  As you have changed the drainage index,'+
              ' please check the subsurface drainage system data group.');
          ...
          for k:=0 to NrOfIntPoly-1 do
            if j<4 then AuxInt[j,k] := StrToInt(Poly_StringGrid.Cells[j,k+1])
              ...
              if AuxInt[2,k]>0 then
                 if Poly_StringGrid.Cells[4,k+1]<>' -' then
...
// Seasons were added, NrOfSeasonsAdded=0
    ...
    NrOfSeasons:=Number;            //not necessary ??   see above
  end;
...
begin
  ...
  begin
    ReadGeneralGroup;                            {for nr. of polygons}
    ReadPolyGroup('Name02',4);                   {for AquiferType}
...
begin
  ...
  DataMod.NrOfitems:=5;
  SetPolyComponents('ToGroupEditing');
  SetPolyScrollBoxStandards ('On');
  NoRead:=false;
  PolyCol_Panel.Visible:=false;
  PolyGroup_ScrollBox.Visible:=false;
  if InputOpened and not Pasted then DataMod.ReadOverallGroup;
//The Overallgroup need not be read when 'DoItSelf' or 'Guided' because the
//data are in the memory
  for k:=0 to DataMod.TotNrOfPoly do
  begin
    if k=0 then
    begin
      PolyGroupEdit[1,k].Text:=' -';
      PolyGroupEdit[2,k].Text:='cm map';
      PolyGroupEdit[3,k].Text:='cm map';
      PolyGroupEdit[4,k].Text:='m level';
      PolyGroupEdit[5,k].Text:='index';
      PolyGroupEdit[1,k].Color:=clBtnFace;
      PolyGroupEdit[2,k].Color:=clBtnFace;
      PolyGroupEdit[3,k].Color:=clBtnFace;
      PolyGroupEdit[4,k].Color:=clBtnFace;
      PolyGroupEdit[5,k].Color:=clBtnFace;
      PolyGroupEdit[1,k].Enabled:=false;
      PolyGroupEdit[2,k].Enabled:=false;
      PolyGroupEdit[3,k].Enabled:=false;
      PolyGroupEdit[4,k].Enabled:=false;
      PolyGroupEdit[5,k].Enabled:=false;
    end else
    if InputOpened or MakeCorrection then with DataMod do
    begin
      PolyGroupEdit[1,k].Text := IntToStr(NodeNr[k-1]);
      PolyGroupEdit[2,k].Text := FloatToStr (Xcoord[k-1]);
      if not GuideStop then PolyGroupEdit[2,k].Enabled:=false
      else PolyGroupEdit[2,k].Enabled:=true;
      PolyGroupEdit[3,k].Text := FloatToStr(Ycoord[k-1]);
      if not GuideStop then PolyGroupEdit[3,k].Enabled:=false
      else PolyGroupEdit[3,k].Enabled:=true;
      if not GuideStop then PolyGroupEdit[4,k].Color:=clSilver
      else PolyGroupEdit[4,k].Color:=$00C6E8FF;
      PolyGroupEdit[4,k].Text := FloatToStr(BottomLevel[k-1]);
      PolyGroupEdit[5,k].Text := IntToStr(Int_Ext_Index[k-1]);
      if not GuideStop then PolyGroupEdit[5,k].Enabled:=false
      else PolyGroupEdit[5,k].Enabled:=true;
    end; {if InputOpened or MakeCorrection then}
  end; {for k:=0 to TotNrOfPoly do-1 do}
  GuideStop:=true;
  ...
  begin
    ...
    begin
      PolyGroupEdit[1,k].Text := '';
      PolyGroupEdit[2,k].Text := '';
      PolyGroupEdit[3,k].Text := '';
      PolyGroupEdit[4,k].Text := '';
      PolyGroupEdit[5,k].Text := '';
    end;
  end;{if InputOpened and (DataMod.NrOfNodesAdded>0)then}
  ...
  PolyGroup_ScrollBox.Visible:=true;
  if DataMod.AnnualCalc and (DataMod.YearNr>0) then
  begin
    AnnualPoly_Memo.Visible:=true;
    PolyGroup_ScrollBox.Enabled:=false;
  end;
...
begin
  ...
  begin
    ...
    Try
      ...
      begin
        NodeNr[k] := StrToInt(PolyGroupEdit[1,k+1].Text);
        Xcoord[k] := StringToFloat(PolyGroupEdit[2,k+1].Text);
        Ycoord[k] := StringToFloat(PolyGroupEdit[3,k+1].Text);
        BottomLevel[k] := StringToFloat(PolyGroupEdit[4,k+1].Text);
        Int_Ext_Index[k] := StrToInt(PolyGroupEdit[5,k+1].Text);
      end;
      Except on E: exception do
      begin
  ...
  begin
    PolyGroupEdit[2,k].Enabled:=true;
    PolyGroupEdit[3,k].Enabled:=true;
    PolyGroupEdit[5,k].Enabled:=true;
  end;
...
begin
  ...
  NrOfPoly:=DataMod.TotNrOfPoly;
  if DataMod.RemovalStage then DataMod.TotNrOfPoly:=DataMod.InitTotNrOfPoly;
  if not Destroyed then SetPolyScrollBoxStandards ('Off');
  if DataMod.RemovalStage then DataMod.TotNrOfPoly:=NrOfPoly;
  Identity:='Network';
  PolyCol_Panel.Visible:=false;
  PolyGroup_ScrollBox.Visible:=false;
  ...
  for k:=0 to DataMod.TotNrOfPoly-1 do
      ...
      begin
        PolyGroupEdit[1,j].Text:=IntToStr(DataMod.NodeNr[k]);
        j:=j+1;
      end;
  for j:=2 to DataMod.NrOfItems do for k:=0 to DataMod.NrOfIntPoly-1 do
      if not DoItSelf then
      begin
        if not GuideEnded then
           PolyGroupEdit[j,k].Enabled:=false
        else PolyGroupEdit[j,k].Enabled:=true;
        if DataMod.NrOfNodesAdded=0 then
           PolyGroupEdit[j,k].Text:=IntToStr(DataMod.Neighbor[j-1,k])
        else
        begin
          if k<DataMod.InitNrOfIntPoly then
             PolyGroupEdit[j,k].Text:=IntToStr(DataMod.Neighbor[j-1,k])
          else
             PolyGroupEdit[j,k].Text:='';
        end;
      end else
      begin
...
procedure TMainForm.NetworkEditExit(Sender: TObject);
{---------------------------------------------------}
label 1;
...
begin
  inherited;
  if (Identity='Network') and DoItSelf then
  for k:=0 to DataMod.NrOfIntPoly-1 do with DataMod do
  begin
    for i:=k to NrOfIntPoly-1 do
    begin
      for j:=i to NrOfIntPoly-1 do
      begin
        for jj:=2 to 7 do
        begin
          if PolyGroupEdit[1,j].Text=PolyGroupEdit[jj,k].Text then
          begin
            Equal:=false;
            for kk:=2 to 7 do
                if PolyGroupEdit[kk,j].Text=PolyGroupEdit[1,k].Text then
                   Equal:=true;
            if not Equal then for ii:=2 to 7 do
            begin
              if (PolyGroupEdit[ii,j].Text='0') or
                 (PolyGroupEdit[ii,j].Text='') then
              begin
                 PolyGroupEdit[ii,j].Text:=PolyGroupEdit[1,k].Text;
                 goto 1;
              end;
            end;
          end;
...
end; {TMainForm.NetworkEditExit}
{--------------------------------}



procedure TMainForm.NodesEditExit(Sender: TObject);
{-------------------------------------------------}
label 1;
var k, NrOfPoly : integer;
    SequenceOK : boolean;
begin
  inherited;
  with DataMod do
  begin
    SequenceOK:=Question('Do you wish to fill the entire column of'+
                          ' node numbers with a sequential series?');
    if SequenceOK then
    begin
      NrOfPoly:=TotNrOfPoly+NrOfNodesAdded;
      for k:=1 to NrOfPoly do
        PolyGroupEdit[1,k].Text:=IntToStr(k);
    end;
  end;
end; {TMainForm.NodesEditExit}
{----------------------------}



procedure TMainForm.BottomEditExit(Sender: TObject);
{--------------------------------------------------}
var k, NrOfPoly  : integer;
    EntireOK : boolean;
begin
  ...
  with DataMod do
  begin
    EntireOK:=Question('Do you wish to fill the entire column '+
                        ' bottom levels with the same value?');
    if EntireOK then
    begin
      NrOfPoly:=TotNrOfPoly+NrOfNodesAdded;
      for k:=1 to NrOfPoly do
          PolyGroupEdit[4,k].Text:=PolyGroupEdit[4,1].Text;
    end;
  end;
end; {TMainForm.BottomEditExit}
{-----------------------------}
...
begin
  ...
  begin
    ...
    Try
      ...
      begin
        if PolyGroupEdit[j,k].Text<>'' then
           HlpVar := StrToInt(PolyGroupEdit[j,k].Text)
        else
          HlpVar:=0;
    ...
    for j:=2 to NrOfItems do for k:=0 to NrOfIntPoly-1 do
    begin
      if (j<5) and (PolyGroupEdit[j,k].Text='') then
      begin
    ...
    for j:= 2 to NrOfItems do for k:=0 to NrOfIntPoly-1 do
    begin
      if (PolyGroupEdit[j,k].Text<>'-') and (PolyGroupEdit[j-1,k].Text='-') then
      begin
    ...
    begin
      Setlength (Neighbor[j-1],NrOfIntPoly);
      for k:=0 to NrOfIntPoly-1 do
      begin
        if (PolyGroupEdit[j,k].Text<>'0') and (PolyGroupEdit[j,k].Text<>'') then
        begin
          Hlpvar := StrToInt(PolyGroupEdit[j,k].Text);
...
begin
  ...
  begin
    ...
    Poly_StringGrid.Width:=8+60*DataMod.NrOfItems;
    Poly_StringGrid.Height:=20+20*Poly_StringGrid.RowCount;
    if Poly_StringGrid.Height>303 then
    begin
      Poly_StringGrid.Width:=28+60*DataMod.NrOfItems;
      Poly_StringGrid.Height:=303;
    end;
    Count:=0;
    for k:=0 to DataMod.NrOfIntPoly do
    begin
...
begin
  ...
  begin
    ...
    PolyHeaderLabel[4].Hint:= 'Rotation key, Kr = 0, 1, 2, 3 or 4, '+
                              ' see "Kr Help"';
...
begin
  ...
  begin
    ...
    PolyHeaderLabel[2].Hint:='Constant inflow condition into the aquifer.'+
                             ' The inflow may come from  an external polygon,'+
                             ' a deeper aquifer, or a geological fault';
    ...
    PolyHeaderLabel[4].Hint:='Constant outflow condition from the aquifer.'+
                             ' The outflow may go into  an external polygon,'+
                             ' a deeper aquifer, or a geological fault';
    for j:=0 to DataMod.NrOfItems-1 do
        for k:=0 to DataMod.NrOfIntPoly do
        begin
...
begin
  ...
  DataMod.NrOfData:=Poly_StringGrid.RowCount;
...
begin
  ...
  begin
    ...
    PolyGroupPanelCancel_Button.Visible:=false;
    ...
    PolyGroupCancel_Button.Visible:=false;
    PolyExplanation1_Edit.Visible:=false;
    PolyCol_Panel.Visible:=false;
  ...
  begin
    ...
    PolyGroupPanelCancel_Button.Visible:=true;
    SetToGroupSelection:=true;
    InputOpen.Enabled:=true;
    PolyHeader_Panel.Visible:=false;
    PolyExplanation1_Label.Visible:=false;
  ...
  begin
    ...
    PolyGroupCancel_Button.Visible:=true;
...
begin
  ...
  Poly_StringGrid.Width:=8+60*DataMod.NrOfItems;
  Poly_StringGrid.Height:=20+20*(NrOfPoly+1);
  if Poly_StringGrid.Height>303 then
  begin
    Poly_StringGrid.Width:=28+60*DataMod.NrOfItems;
    Poly_StringGrid.Height:=303;
...
procedure TMainForm.SetPolyScrollBoxStandards (const Task : string);
{--------------------------------------------------------------------------}
var j, k, NrOfLines, NrOfPoly : integer;
begin
  with DataMod do
  begin
    NrOfPoly:=DataMod.NrOfIntPoly;
    if (Identity='Overall') or (Identity='Finished') then
        NrOfPoly:=DataMod.TotNrOfPoly;
    NrOfItems:=5;
  end;
  ...
  begin
    ...
    Poly_StringGrid.Visible:=false;
    PolyGroup_ScrollBox.Visible:=false;
    PolyHeader_Panel.Width:=7+60*DataMod.NrOfItems;
    for j:=1 to DataMod.NrOfItems do
    begin
      PolyHeaderLabel[j]:=Tlabel.Create(self);
      PolyHeaderLabel[j].Parent:=PolyHeader_Panel;
    ...
    NrOfLines:=NrOfPoly;
    if Identity='Overall' then NrOfLines:=NrOfPoly+1;
    for j:=1 to DataMod.NrOfItems do
        setlength (PolyGroupEdit[j],NrOfLines+1);
    for j:=1 to DataMod.NrOfItems do for k:=0 to NrOfLines-1 do
    begin
      PolyGroupEdit[j,k]:=Tedit.Create(Self);
      PolyGroupEdit[j,k].Parent:=PolyGroup_ScrollBox;
      PolyGroupEdit[j,k].Height:=20;
      PolyGroupEdit[j,k].Width:=60;
      PolyGroupEdit[j,k].Top:=20*k;
      PolyGroupEdit[j,k].Left:=0;
      if j=1 then
      begin
        PolyGroupEdit[j,k].ReadOnly:=true;
        PolyGroupEdit[j,k].Enabled:=false;
        PolyGroupEdit[j,k].Color:=clBtnFace;
        if (Identity='Overall') and DoItSelf then
        begin
          PolyGroupEdit[j,k].ReadOnly:=false;
          PolyGroupEdit[j,k].Enabled:=true;
          PolyGroupEdit[j,k].Color:=clWhite;
        end;
        if (Identity='Overall') and not DoItSelf
                                and (DataMod.NrOfNodesAdded>0) then
        begin
          if k>NrOfLines-DataMod.NrOfNodesAdded-1 then
          begin
             PolyGroupEdit[j,k].ReadOnly:=false;
             PolyGroupEdit[j,k].Enabled:=true;
             PolyGroupEdit[j,k].Color:=clWhite;
        ...
        if (Identity='Overall') and not DoItSelf
                                and (DataMod.NrOfNodesAdded=0) then
            PolyGroupEdit[j,k].Color:=clBtnFace;
        if (Identity='Overall') and (k=1) then
          PolyGroupEdit[j,k].OnExit:=NodesEditExit;
      end
      else {j>1}
      begin
        PolyGroupEdit[j,k].Color:=$00C6E8FF;
        if (Identity='Overall') and (k=1) and (j=4) then
            PolyGroupEdit[j,k].OnExit:=BottomEditExit;
        PolyGroupEdit[j,k].ReadOnly:=false;
        PolyGroupEdit[j,k].Left:=60*(j-1);
        if j=2 then
           if GuidedOK then PolyGroupEdit[j,k].Enabled:=false
           else PolyGroupEdit[2,k].Enabled:=true;
        if j=3 then
           if GuidedOK then PolyGroupEdit[3,k].Enabled:=false
           else PolyGroupEdit[3,k].Enabled:=true;
      end;
    end; {for j:=1 to DataMod.NrOfItems do with DataMod do}
    PolyGroup_ScrollBox.Width:=6+60*DataMod.NrOfItems;
    PolyGroup_ScrollBox.Height:=4+21*NrOfLines;
    if PolyGroup_ScrollBox.Height>305 then
    begin
      PolyGroup_ScrollBox.Width:=22+60*DataMod.NrOfItems;
      PolyGroup_ScrollBox.Height:=305;
    end;
    PolyGroup_ScrollBox.Visible:=true;
  end; {if Task='On'}

  if (Task='Off') and not Destroyed then
  begin
    PolyGroup_ScrollBox.Visible:=false;
    NrOfLines:=NrOfPoly;
    if (Identity='Overall') or (Identity='Finished') then NrOfLines:=NrOfPoly+1;
    for j:=1 to DataMod.NrOfItems do for k:=0 to NrOfLines-1 do
        Try
          PolyGroupEdit[j,k].Destroy;
          Except on E: exception do
          begin
//            Showmessage ('There is a problem .... ');
    ...
    if not Critical then Identity:='None';
    Destroyed:=true;
  end;

  end;{TMainForm.SetPolyScrollBoxStandards}
{-----------------------------------------}
...
procedure TMainForm.PolyGroupPanelCancel_ButtonClick(Sender: TObject);
{--------------------------------------------------------------------}
var AbandonOK : boolean;
begin
  inherited;
  ...
  GeneralInput_Tabsheet.TabVisible:=true;
  GeneralInput_Tabsheet.Show;
end; {TMainForm.PolyGroupPanelCancel_ButtonClick}
{-----------------------------------------------}
...
//Gives -1 values when a value is n.a. omdat AreaA or AreaB or AreaU are zero
    ...
    if ChangeOfSeasonDuration then
       Showmessage ('You have changed the season durations.'+
                   '  Adjust all seasonal data correspondingly.');
  end;
...
begin
  ...
  begin
    if SeasonalInput_Tabsheet.TabVisible then
    for j:=1 to NrOfSeasons + NrOfSeasonsAdded do
    begin
      TmpDuration[j]:=SeasonDuration[j];
      Try
        SeasonDuration[j]:=StrToInt(Season_StringGrid.Cells[1,j]);
        Except on E: exception do
        begin
          Showmessage ('The season duration is not a true integer');
    ...
    begin
      SaveGeneralGroup(DataOK);
      if not DataOK then exit;
      Showmessage ('The season duration data were checked and saved');
    end else
    begin
       for j:=1 to NrOfSeasons do
           Season_StringGrid.Cells[1,j]:=FloatToStr(TmpDuration[j]);
...
procedure TMainForm.ShowDurationsTable;
{-------------------------------------}
var j,k : integer;
begin
  with DataMod do
  begin
    DataMod.NrOfItems:=2;
    SeasonHeader_Panel.Width:=5+80*DataMod.NrOfItems;
    for j:=1 to DataMod.NrOfItems do
    begin
      SeasonHeaderLabel[j]:=Tlabel.Create(self);
      SeasonHeaderLabel[j].Parent:=SeasonHeader_Panel;
      SeasonHeaderLabel[j].Top:=3;
      SeasonHeaderLabel[j].Width:=80;
      SeasonHeaderLabel[j].Height:=40;
      if j=1 then SeasonHeaderLabel[j].Left:=4
      else SeasonHeaderLabel[j].Left:=SeasonHeaderLabel[j-1].Left+80;
      SeasonHeaderLabel[j].OnMouseEnter := ShowExplainEdit3;
      SeasonHeaderLabel[j].OnMouseLeave := HideExplainEdit3;
    end;
    Season_StringGrid.DefaultColWidth:=80;
    Season_StringGrid.DefaultRowHeight:=40;
    if Identity='Durations' then
    begin
       Season_StringGrid.RowCount:=1+DataMod.NrOfSeasons;
       Season_StringGrid.Height:=20+40*(DataMod.NrOfSeasons+1);
    end else
    begin
       Season_StringGrid.RowCount:=1+DataMod.NrOfSeasons*DataMod.NrOfIntPoly;
       Season_StringGrid.Height:=
                         20+20*(DataMod.NrOfSeasons*DataMod.NrOfIntPoly+1);
    end;
    Season_StringGrid.ColCount:=DataMod.NrOfItems;
    Season_StringGrid.Width:=8+80*DataMod.NrOfItems;
    Season_StringGrid.FixedCols:=1;
    Season_StringGrid.FixedRows:=1;
    SeasonHeaderLabel[1].Caption:='  Season         ';
    SeasonHeaderLabel[2].Caption:='  Duration       ';
    SeasonHeaderLabel[1].Hint:='Season number depending on the number of'+
                               ' seasons per year, see general data';
    SeasonHeaderLabel[2].Hint:='Duration of the season in months (integer),'+
                               '  minimum 1 month';
    for j:=0 to 1 do
      for k:=0 to DataMod.NrOfSeasons do
      begin
        if k=0 then
        begin
          Season_StringGrid.Cells[0,k]:=' Number ';
          Season_StringGrid.Cells[1,k]:=' Months ';
        end;
        if k>0 then
        begin
          Season_StringGrid.Cells[0,k]:= IntToStr(k);
          Season_StringGrid .Cells[1,k] := FloatToStr(SeasonDuration[k]);
        end;
      end;
  end; {with DataMod do}
  SeasonCol_Panel.Visible:=false;
  Season_StringGrid.Visible:=true;
end; {TMainForm.ShowDurationsTable}
{---------------------------------}
...
begin
  ...
  begin
    ...
    SeasGroupCancel_Button.Visible:=false;
    SeasExplanation1_Edit.Visible:=false;
    SeasonCol_Panel.Visible:=false;
  ...
  begin
    ...
    SeasGroupCancel_Button.Visible:=true;
    SeasonHeader_Panel.Visible:=true;
...
begin Inherited;
      ...
      begin
        ...
        OutputSymbols_Button.Enabled:=true;
        OutputSelection_Button.Visible:=true;
        OutputSelection_Button.Enabled:=false;
      end;
      ...
      begin
        OutputSelection_Button.Visible:=true;
        OutputSymbols_Button.Visible:=true;
      end;
...
begin
  ...
  begin
    ...
    GoToInput_Button.Visible:=true;
    SaveGroup_Button.Enabled := false;
    GoToInput_Button.Enabled := false;
  end;
  SeeGraph_Button.Enabled := false;
  OutputSalt_Image.Visible := false;
  if not InputOpened then DeleteGroupFiles;        {GroupFiles are "Name"files}
  if DirectoryExists (DataDir) then chdir (DataDir);
  if not InputOpened then DeleteGroupFiles;
  with DataMod do
  begin
...
begin
  ...
  SeeGraph_Button.Enabled := false;
  SaveGroup_Button.Enabled:=false;
  if InputOpened then GotoInput_Button.Enabled:=false;
  Output_Memo.Visible := false;
  OutputSelection_Button.Enabled:=false;
...
procedure TMainForm.Time_RadioButtonClick(Sender: TObject);
begin
  inherited;
  TimeSelection_Panel.Visible:=false;
  TypeSelection_Button.Visible:=true;
...
procedure TMainForm.Poly_RadioButtonClick(Sender: TObject);
begin
  inherited;
  PolySelection_Panel.Visible:=false;
  TypeSelection_Button.Visible:=true;
...
begin
  inherited;
    ...
    PolyOK:=false;
    if InputOpened and CalcDone then
    begin
       for k:=1 to DataMod.NrOfIntPoly do
           if DataMod.PolyWanted=DataMod.NodeNr[k] then PolyOK:=true
    end else if OutputOpened then
    begin
...
begin
  OutputSelection_Button.Enabled:=true;
  SaveGroup_Button.Enabled := true;
  if not InputOpened then GotoInput_Button.Enabled:=true;
  OutputShowMap_Button.Visible:=false;
  Mapping_Button.Visible := false;
  OutputSymbols_Button.Enabled := true;
  FillOutputWaitMemo;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        for jj:=0 to DataMod.NrOfData-1 do
            DataMod.Variable[1,jj]:= -DataMod.Variable[1,jj];
    ...
    if Evapo_RadioButton.Checked then GroupMark := 'EaU';
    if CropArea_RadioButton.Checked then GroupMark := 'A';
    if (GroupMark<>'Dw') and (GroupMark<>'Zs') then
    begin
      if PolyData then
      begin
...
begin
  ...
  begin
    ...
    closefile (TmpFile);
    for i:=1 to 7 do PosValues[i]:=false;
    ...
    begin
      FileLine[2] := ('"Actual evaporation from unirrigated land '+
                      ' EaU in m/season,"');
    ...
    begin
      ...
      begin
        ...
        if GroupMark='EaU' then if i=6 then
           FileLine[k+6]:= FileLine[k+6]+MainStr[6];
...
begin
  ...
  begin
    ...
    begin
      ...
      for i:= 1 to DataMod.NrOfitems do
      begin
        if (GroupMark<>'Dw') and (GroupMark<>'Gti') then
        begin
          if (DataMod.Variable[i,k-1]<0) and PosValues[i] then
            MainStr[i]:='    -    ';
          if (DataMod.Variable[i,k-1]<0) and not PosValues[i] then
...
begin
  inherited;
    ...
    for k:=1 to 7 do PosValues[k]:=false;   {Only PosValues 1, 5 and 6 are used}
    ...
    if DataMod.GroupMark='Gti' then for i:=1 to DataMod.NrOfitems do
       PosValues[i]:=true;
    if DataMod.GroupMark='EaU' then PosValues[6]:=PosValues[1];
    GroupOpened:=true;
    OutputSelection_Button.Enabled:=false;
    GotoInput_Button.Enabled:=true;
    OutputSymbols_Button.Enabled:=true;
...
begin
  ...
  begin
    Showmessage ('The number of internal polygons is greater than 20.'+
                 ' Select maximum 20 polygons in a suitable cross-section'+
                 ' in the next table.');
    SelectPolyForm.Show;
  end else
...
begin
  ...
  if (DataMod.GroupMark='EaU') and not PosValues[6] then
  begin
...
// Y and X axes
    ...
    begin
      if NrOfIntPoly>20 then
      begin
        setlength (NrOfNode,NumberWanted);
        for jj:=0 to NumberWanted-1 do
            NrOfNode[jj]:=PolySelected[jj];
      end else
      begin
        setlength (NrOfNode,NrOfIntPoly);
        for jj:=0 to NumberWanted-1 do
            NrOfNode[jj]:=NodalNr[jj];
      end;
      for jj:=0 to NumberWanted-1 do
      begin
      ...
      begin
        for jj:=0 to NrOfYears do
          ...
          begin
            ...
            if Count<NrOfData then
            begin
              Moveto (RelXdata[Count-1],RelYdata[6,Count-1]);
              Lineto (RelXdata[Count-1],RelYdata[6,Count]);
              Moveto (RelXdata[Count-1],RelYdata[6,Count]);
              Lineto (RelXdata[Count],RelYdata[6,Count]);
            end;
          end;
      end else
      begin
        ...
        if jj<NumberWanted-1 then
        begin
          Moveto (RelXdata[jj],RelYdata[6,jj]);
          Lineto (RelXdata[jj+1],RelYdata[6,jj]);
        end else
        begin
          Moveto (RelXdata[jj],RelYdata[6,jj]);
          Lineto (RelXdata[jj]+2*Middle,RelYdata[6,jj]);
        end;
      end; {if not PolyData then else then}
      Font.Color := clMaroon;     {for the text}
    end;{if GroupMark='EaU' then}
