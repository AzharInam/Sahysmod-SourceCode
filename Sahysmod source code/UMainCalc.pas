...
//Sub-procedures under procedure Saltbalances
  procedure AuxSB1 (var INFAx,INFBx,INFUx,IRAx,IRBx,RAINx,
            SCMAx,SCMBx,SCMUx,SCXx,SDAx,SDBx,SDLAx,SDLBx,SDLUx,
            SRIx,SROx : real; ii : integer);
  procedure AuxSB2 (var AUB,SCR1,SCM1,SC01,SCR2,SCM2,SC02,TSC1,TSC2,TSI1,TSI2,
            TPL1,TPL2: real; ii : integer);
  procedure AuxSB3 (var PLF,SCRAx,SCMAx,TSIx : real;
                    ii : integer);
...
implementation
  ...
  var MonthlySteps : integer;
      ...
      SCRA0, SCRB0, SCRU0  : real;
      Again, Error         : boolean;
      ReUseWarning, ReUseReduced : boolean;
      IrrWarning, IrrReduced : boolean;
      AreaWarning, AreaReduced : boolean;
      StoreFile            : textfile;

procedure  MainCalculations;
label 1 ;
var i, j, ij, K, M, Start, Year, Y, Days, Number, CountIt, Advance,
    OutputIndex        : integer;
    HF0, HFR           : array of real;
    NrOfDays, Wel0, CF : real;
    Progr1, Progr2     : boolean;
    WellWarning, WellReduced : boolean;
    WellTxt            : string;
...
//Setting values of intial and reference Waterlevel
  for i:=1 to NrOfIntPoly do
  begin
    setlength (HF0,i+1);
    setlength (HFR,i+1);
...
//to the soil surface
          if DWT<0.35 then ReduceIrr (Y,K);

//Reference value of WellDischarge
          Wel0:=WellDis[Y,K];
//At the end of the season: reducing the pumping from wells when the waterlevel
//drops
          if not WellWarning and (WaterLevel[K]<0.9*HF0[K]) and
             (WaterLevel[K]<HFR[K]) and (Wel0>0) then
          begin
              Warning :=true;
              ...
              WellTxt := ' As the water table drops in polygon: '+WellTxt+
                         ', the well discharge is reduced here.';
...
//Salt accumulation above the soil surface and adjustment of ZIR
  ...
  if (SCrA[i]>51) AND (SurfDrA[j,i]<0.01) THEN SDAA:=0.01*(SCrA[i]-50);
  if (SCrB[i]>51) AND (SurfDrB[j,i]<0.01) THEN SDBB:=0.01*(SCrB[i]-50);
  if (SCrU[i]>51) AND (SurfOutflow[j,i]<0.01) THEN SROO:=0.01*(SCrU[i]-50);
  ...
  Zinf:=ZIRR*IrrT+SCRy*UPWA;
  Zout:=ZIR0*SDrT+ZIR*DOWN;
  SALY[i]:=SALY[i]+Zinf-Zout;                  {Storage of salt on the surface}
  IF (DWT<0) and (SALY[i]>0) THEN ZIR:=ZIRR+SALY[i]/(WaterIn-DWT);
//In RunSSM:  IF DWT<0 THEN ZIR:=(Zinf+SALY[i])/(WaterIn-DWT);
  IF (DWT>=0) AND (Dwt0>=0) THEN SALY[i]:=0;
  ZIR0:=ZIR;
...
//and in the unirrigated area (SCRU) when RotationType[i]=0
  IF RotationType[i]=0 THEN
  BEGIN
    AuxSB1 (INFA,INFB,INFU,IrrA[j,i],IrrB[j,i],RAIN[j,i],SCMA,SCMB,SCMU,SCX,
            SDAA,SDBB,SDLA,SDLB,SDLU,SurfInflow[j,i],SROO,i);
...
//rotationally irrigated A and B area (SCrA) when RotationType=1
  ...
  BEGIN
    TSIU:=INFU*ZIR-SurfOutflow[j,i]*(SDLU*SCrU[i]+ZIR);
    IF (DWT>=0) AND (Dwt0<0) THEN TSIU:=TSIU+SALY[i];
    TSCU:=CAU*LeachEffTr[i]*SCX;
    IF DWT<0 THEN TSCU:=TSCU-UPWA*LeachEffRo[i]*SCrU[i];
    IF UCarea[i]<0.999 THEN
    BEGIN
      TSI:=(TIR-INFU*UCarea[i])*ZIR;
      TSO:=(TDR-SurfOutflow[j,i]*UCarea[i])*(SDLA*SCrA[i]+ZIR);
      TSIA:=(TSI-TSO)/(1-UCarea[i]);
      IF (DWT>=0) AND (Dwt0<0) THEN TSIA:=TSIA+SALY[i];
      TCAA:=(CAT-CAU*UCarea[i])/(1-UCarea[i]);
      TSCA:=TCAA*LeachEffTr[i]*SCX;
      IF DWT<0 THEN TSCA:=TSCA-UPWA*LeachEffRo[i]*SCrA[i];
...
//With short time steps of 1 day AuxSB2 with iterations is not really required
    ACCUM:=TSIA-TPLA*LeachEffRo[i]*SCRA[i]+TSCA;
    SCRA[i]:=SCRA[i]+ACCUM/(TotPorRo[i]*RootZone[i]);
    ACCUM:=TSIU-PLU*LeachEffRo[i]*SCRU[i]+TSCU;
    SCRU[i]:=SCRU[i]+ACCUM/(TotPorRo[i]*RootZone[i]);

//Total salt percolating
//    TSP:=LeachEffRo[i]*(TPLA*SCMA*(1-UCarea[i])+PLU*SCMU*UCarea[i]);
    TSP:=LeachEffRo[i]*(TPLA*SCRA[i]*(1-UCarea[i])+PLU*SCRU[i]*UCarea[i]);

  END; {IF RotationType[i]=1 THEN}

  IF RotationType[i]=2 THEN
  BEGIN
    TSIA:=INFA*ZIR-SurfDrA[j,i]*(SDLA*SCrA[i]+ZIR);
    IF (DWT>=0) AND (Dwt0<0) THEN TSIA:=TSIA+SALY[i];
    ...
    BEGIN
      ...
      TSO:=(TDR-SurfDrA[j,i]*ACarea[i])*(SDLA*SCrA[i]+ZIR);
      TSIB:=(TSI-TSO)/(1-ACarea[i]);
      IF (DWT>=0) AND (Dwt0<0) THEN TSIB:=TSIB+SALY[i];
      TCAB:=(CAT-CAA*ACarea[i])/(1-ACarea[i]);
      TSCB:=TCAB*LeachEffTr[i]*SCX;
      IF DWT<0 THEN TSCB:=TSCB-UPWA*LeachEffRo[i]*SCrB[i];
...
//With short time steps of 1 day AuxSB2 with iterations is not really required
    ACCUM:=TSIB-TPLB*LeachEffRo[i]*SCRB[i]+TSCB;
    SCRB[i]:=SCRB[i]+ACCUM/(TotPorRo[i]*RootZone[i]);
    ACCUM:=TSIA-PLA*LeachEffRo[i]*SCRA[i]+TSCA;
    SCRA[i]:=SCRA[i]+ACCUM/(TotPorRo[i]*RootZone[i]);
//Total salt percolating
//    TSP:=LeachEffRo[i]*(PLA*SCMA*ACarea[i]+TPLB*SCMB*(1-ACarea[i]));
    TSP:=LeachEffRo[i]*(PLA*SCRA[i]*ACarea[i]+TPLB*SCRB[i]*(1-ACarea[i]));
  END; {IF RotationType[i]=2 THEN}

  IF RotationType[i]=3 THEN
  BEGIN
    TSIB:=INFB*ZIR-SurfDrB[j,i]*(SDLB*SCrB[i]+ZIR);
    IF (DWT>=0) AND (Dwt0<0) THEN TSIB:=TSIB+SALY[i];
    ...
    BEGIN
      ...
      TSO:=(TDR-SurfDrB[j,i]*BCarea[i])*(SDLB*SCrB[i]+ZIR);
...
//With short time steps of 1 day AuxSB2 with iterations is not really required
    ACCUM:=TSIA-TPLA*LeachEffRo[i]*SCRA[i]+TSCA;
    SCRA[i]:=SCRA[i]+ACCUM/(TotPorRo[i]*RootZone[i]);
    ACCUM:=TSIB-PLB*LeachEffRo[i]*SCRB[i]+TSCB;
    SCRB[i]:=SCRB[i]+ACCUM/(TotPorRo[i]*RootZone[i]);
//Total salt percolating
...
//Salt concentration in the rootzone (SCRA) when full rotation occurs
  ...
  BEGIN
    TSI:=TIR*ZIR+CAT*LeachEffTr[i]*SCX-TDR*(SDLA*SCrA[i]+Zir);
    IF DWT<0 THEN TSI:=TSI-UPWA*LeachEffRo[i]*SCrA[i];
    IF (DWT>=0) AND (Dwt0<0) then TSI:=TSI+SALY[i];
//    AuxSB3 (PLT,SCrA[i],SCMA,TSI,i);
//With short time steps of 1 day AuxSB2 with iterations is not really required
    ACCUM:=TSI-PLT*LeachEffRo[i]*SCRA[i];
    SCRA[i]:=SCRA[i]+ACCUM/(TotPorRo[i]*RootZone[i]);
//Total salt percolating
//    TSP:=LeachEffRo[i]*PLT*SCMA;
    TSP:=LeachEffRo[i]*PLT*SCRA[i];
  END;
...
//Final corrections
  IF (DWT>=0) AND (Dwt0<0) THEN ZIR:=ZIRR+SALY[i]/(TIR+RAIN[j,i]-TDR-Dwt0);
  IF (DWT<0) AND (Dwt0>0) THEN ZIR:=ZIRR;
  NoIrr:=false;
...
begin
  ...
  begin
    ...
    Rain[j,n]:=Rain0[j,n]/(MonthlySteps*SeasonDuration[j]);
...
//and drainage, to obtain seasonal values
          CitT[i]:=CitT[i]+Cit;             {irrigation water including re-use}
          CirT[i]:=CirT[i]+Cir;             {irrigation water excluding re-use}
...
//into seasonal values
          ...
          ZIRT[i]:=ZIRT[i]+ZIR*CIT;          {amount of salt brought in by Irr}
          ZWLT[i]:=ZWLT[i]+
...
//has been increased and DWT has therefore increased
  ...
  begin
    ...
    IrrReduced := true;
    PolyTxt := inttostr (i);
    PolyTxt := ' As the watertable rises above the soil surface'+
...
procedure FarmResponses (j, i : integer);
var AI, BI, FA, FB, FAC, DWTC, IRAI, IRBI : real;
    PolyTxt : string;
begin with DataMod do
begin
...
//Reducing the seasonal irrigated B area
  ...
  begin
    ...
    WarningForm.WarningMemo.Lines.Add
                  (' Irrigated area is reduced as a response of the farmers');
    WarningForm.WarningMemo.Lines.Add (PolyTxt);
    WarningForm.WarningMemo.Lines.Add ('');
  end;
...
procedure AuxSB1 (var INFAx,INFBx,INFUx,IRAx,IRBx,RAINx,SCMAx,SCMBx,SCMUx,SCXx,
                  SDAx,SDBx,SDLAx,SDLBx,SDLUx,SRIx,SROx : real; ii : integer);
{-----------------------------------------------------------------------------}
label 1,2,3;
var SCRIA, SCRIB, SCRIU, ACC, ACCI, ACCO : real;
...
1: ACCI:=IRAx*ZIR+CAA*LeachEffTr[ii]*SCXx;
   IF DWT<0 THEN ACCI:=ACCI-CAA*LeachEffRo[ii]*SCRIA;
   IF (DWT>=0) AND (Dwt0<0) THEN ACCI:=ACCI+SALY[ii];
   ACCO:=SDAx*(SDLAx*SCRIA+ZIR)+SCRIA*LeachEffRo[ii]*PLA;
   ACC:=ACCI-ACCO;
...
//Finding the average salt concentration of rootzone of A stepwise
   ...
   BEGIN
     ...
     SCRIA:=SCRIA-0.01*SCRA0;
     IF SCRIA<=SCMAx THEN GOTO 2;
     GOTO 1;
   END ELSE
...
2: ACCI:=IRBx*ZIR+CAB*LeachEffTr[ii]*SCXx;
   IF DWT<0 THEN ACCI:=ACCI-CAB*LeachEffRo[ii]*SCRIB;
   IF (DWT>=0) AND (Dwt0<0) THEN ACCI:=ACCI+SALY[ii];
   ACCO:=SDBx*(SDLBx*SCRIB+ZIR)+SCRIB*LeachEffRo[ii]*PLB;
   ACC:=ACCI-ACCO;
...
//Finding the average salt concentration of rootzone of B stepwise
   ...
   BEGIN
     ...
     SCRIB:=SCRIB-0.01*SCRB0;
     IF SCRIB<=SCMBx THEN GOTO 3;
     GOTO 2;
   END ELSE
...
3: ACCI:=SRIx*ZIR+CAU*LeachEffTr[ii]*SCXx;
   IF DWT<0 THEN ACCI:=ACCI-CAU*LeachEffRo[ii]*SCRIU;
   IF (DWT>=0) AND (Dwt0<0) THEN ACCI:=ACCI+SALy[ii];
   ACCO:=SROx*(SDLUx*SCRIU+ZIR)+SCRIU*LeachEffRo[ii]*PLU;
   ACC:=ACCI-ACCO;
...
//Finding the average salt concentration of rootzone of B stepwise
   ...
   BEGIN
     ...
     SCRIU:=SCRIU-0.01*SCRU0;
     IF SCRIU<=SCMUx THEN exit;
     GOTO 3;
   END ELSE
     SCMBx:=(SCRB0+SCrB[ii])/2;
...
1: ACC:=TSI1-TPL1*LeachEffRo[ii]*SRI1+TSC1;
...
//Finding the average salt conc. of rootzone stepwise, mixed area
   IF ACC<0 THEN
   BEGIN
     if SCR1<=0 then SCR1:=0.001;
     IF SCR1<=0 THEN SCR1:=0.1*SC01;
     SCM1:=sqrt(SC01*SCR1);
     SRI1:=SRI1-0.01*SC01;
     IF SRI1<=SCM1 THEN GOTO 2;
     GOTO 1;
   END ELSE
     SCM1:=(SC01+SCR1)/2;
...
3: ACC:=TSI2-TPL2*LeachEffRo[ii]*SRI2+TSC2;
...
//Finding the average salt conc. in the rootzone stepwise, fixed area
   IF ACC<0 THEN
   BEGIN
     if SCR2<=0 then SCR2:=0.001;
     IF SCR2<=0 THEN SCR2:=0.1*SC02;
     SCM2:=sqrt(SC02*SCR2);
     SRI2:=SRI2-0.01*SC02;
     IF SRI2<=SCM2 THEN exit;
     GOTO 3
   END ELSE
     SCM2:=(SC02+SCR2)/2;
...
1: ACCA:=TSIx-PLF*LeachEffRo[ii]*SCIA;
...
//area stepwise
   ...
   BEGIN
     ...
     SCIA:=SCIA-0.01*SCRA0;
     IF SCIA<=SCMAx THEN exit;
     GOTO 1;
   END ELSE
     SCMAx:=(SCRA0+SCRAx)/2;
...
1: ACC:=TSPx+PLC*ZIR+VUSx*LeachEffAq[ii]*SCaq[ii]
        -(CAT+VDDx)*LeachEffTr[ii]*SCTT;
   IF DWT>=RootZone[ii]+TranZone[ii] THEN
      ACC:=TSPx-PLT*LeachEffTr[ii]*SCTT-CAT*(LeachEffTr[ii]*SCTT
...
{---------------------------------------------------------------------------}
label 1, 2, 3;
var VUX, VDX, VAX, VUB, SCUU, SCI, ACC, PO, SCLL, TR : real;
...
//the transition zone
1: ACC:=TSPx+VUX*LeachEffTr[ii]*SCtL[ii]-VAX*LeachEffTr[ii]*SCUU;
   IF DWT>=DrainDepth[ii] THEN
      ACC:=TSPx-PLT*LeachEffTr[ii]*SCUU+CAT*LeachEffTr[ii]*(SCtL[ii]-SCtU[ii]);
...
3: ACC:=VDX*LeachEffTr[ii]*SCUMx+PLC*ZIR+VUSx*LeachEffAq[ii]*SCaq[ii]
        -VUB*LeachEffTr[ii]*SCLL;
   IF DWT>=RootZone[ii]+TranZone[ii] THEN
      ACC:=PLT*LeachEffTr[ii]*(SCUMx-SCLL)+CAT*(LeachEffAq[ii]*SCaq[ii]
           -LeachEffTr[ii]*SCLL);
...
{---------------------------------------------------------------}
label 1;
var ACC,ACCU,SCAA,SCI,SCM,SCX : real;
...
1: ACCU:=HIG*ZIG-(HOG+WELx*LeachEffAq[ii])*SCAA;
  ACC:=ACCU+VDDx*LeachEffTr[ii]*SCX-VUSx*LeachEffAq[ii]*SCAA;
  IF DWT>RootZone[ii]+TranZone[ii]
     THEN ACC:=ACCU+PLF*LeachEffTr[ii]*SCX+PLC*ZIR-CAT*LeachEffAq[ii]*SCAA;
...
//Finding the average salt concentration in the aquifer stepwise
  ...
  BEGIN
    ...
    SCM:=sqrt(SCI*SCaq[ii]);
    SCAA:=SCAA-0.01*SCI;
    IF SCAA<SCM THEN exit;
    GOTO 1;
  END;
