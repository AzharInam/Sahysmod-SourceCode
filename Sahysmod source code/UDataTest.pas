...
var InputFile, ErrorFile                           : textfile;
    RotationType, Counter, SerialNodeNr,
    NrOfNextNodes                                  : array of integer;
    ...
    SCrA, SCrB, SCrU, SCtU, SCtL, SCtr, SCaq,
    ACarea, BCarea, UCarea, Qinf, Cinf, Qout,
    WaterLevel, WaterPress, HF0                    : array of real;
...
//Checking keys for farming practices
  ...
  begin
    ...
    if (RotationType[i]<0) or (RotationType[i]>4) then ii:=1;
...
//Checking salinity of irrigation water
  ...
  if ii=1 then
     writeln (ErrorFile,' The salinity Cic of the irrigation water must be >0');
...
//Checking initial waterlevels and aquifer depth
  ...
  begin
    ...
    AquiZone[i]:=SurfLevel[i]-RootZone[i]-TranZone[i];
...
procedure Geometry;
var SUM, TotArea, Dummy, X, Y, A, B, C, D, E,
