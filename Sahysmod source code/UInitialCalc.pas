...
interface
  ...
  var Qaq, Byp, EpU0, AreaU, AA0, BB0,
      IRA0, IRB0, EpA0, EpB0, Rain0         : array of array of real;
      SALY, Dw0, StorWL                     : array of real;
...
//in unit UMainCalc
  ...
  setlength (Rain0,NrOfSeasons+1,NrOfIntPoly+1);
...
//Setting permanent areas A, B and U, adjusting the rotation type accordingly
  AdjustRotation;
...
//Repetition from procedure TestData in UDataTest, but required more often
  ...
  begin
    ...
    begin
      ...
      if SUM<UCArea[i] then UCarea[i]:=SUM;
    end;
  end;
//Changing rotation type if required
  for i:=1 to NrOfIntPoly do
  begin
    Sum:=ACarea[i]+BCarea[i]+UCarea[i];
    RotationType[i]:=4;
    if (ResponsIndex[i]=0) or ((ResponsIndex[i]=1) and (InclIndex[i]=0)) then
    begin
      if (InitRota[i]=0) then if (ACarea[i]>0.999) or (BCarea[i]>0.999)
         or (UCarea[i]>0.999) then RotationType[i]:=0;
      if (InitRota[i]=1) and (UCarea[i]>0) then RotationType[i]:=1;
      if (InitRota[i]=2) and (ACarea[i]>0) then RotationType[i]:=2;
      if (InitRota[i]=3) and (BCarea[i]>0) then RotationType[i]:=3;
    end;

    if (InitRota[i]=0) and (Sum>0.999) then RotationType[i]:=0;
    if (InitRota[i]=1) and (Sum>0.999) then
        if (UCarea[i]>0.001) then RotationType[i]:=1;
    if (InitRota[i]=2) and (Sum>0.999) then
        if (ACarea[i]>0.001) then RotationType[i]:=2;
    if (InitRota[i]=3) and (Sum>0.999) then
        if (BCarea[i]>0.001) then RotationType[i]:=3;
...
//Setting initial values of soil salinities depending on rotation
  InitSalinity;
//Adjusting salinity settings
  if not RotaPassed then for i:=1 to NrOfIntPoly do
  begin
    Sum:=1-AreaA[1,i]-AreaB[1,i];
    if (RotationType[i]=0) then
        if (ACarea[i]>=BCarea[i]) and (Sum<0.001) then SCrU[i]:=SCrA[i];
    if (RotationType[i]=0) then
        if (ACarea[i]<BCarea[i]) and (Sum<0.001) then SCrU[i]:=SCrB[i];
...
begin
  ...
  begin
    DA:=ACarea[i]-AreaA[1,i];
    DB:=BCarea[i]-AreaB[1,i];
    Sum:=1-AreaA[1,i]-AreaB[1,i];
    DU:=UCarea[i]-Sum;
    if RotationType[i]=1 then
    begin
      if DU>0 then SCrU[i]:=(Sum*SCrU[i]+DU*SCrA[i])/(Sum-ACarea[i]);
      SCrA[i]:=SCrU[i];
      if (DA<0) and (UCarea[i]<1) then
          SCrA[i]:=((1-Sum)*SCrA[i]-DU*SCrU[i])/(1-UCarea[i]);
    end;
    if RotationType[i]=2 then
    begin
      if DA>0 then SCrA[i]:=(AreaA[1,i]*SCrA[i]+DA*SCrB[i])/(AreaA[1,i]+DA);
      SCrB[i]:=SCrA[i];
      if (DA<0) and (ACarea[i]<1) then
          SCrB[i]:=((1-AreaA[1,i])*SCrB[i]-DB*SCrA[i])/(1-ACarea[i]);
    end;
    if RotationType[i]=3 then
    begin
      if DB>0 then SCrB[i]:=(AreaB[1,i]*SCrB[i]+DB*SCrA[i])/(AreaB[1,i]+DB);
      SCrA[i]:=SCrB[i];
      if (DB<0) and (BCarea[i]<1) then
          SCrA[i]:=((1-AreaB[1,i])*SCrA[i]-DB*SCrB[i])/(1-BCarea[i]);
    end;
    ...
    begin
      if (RotationType[i]=1) then
         if (Sum>0.999) then
            ...
            SCrA[i]:=(AreaA[1,i]*ScrA[i]+AreaB[1,i]*SCrB[i])/(1-Sum);
      if (RotationType[i]=2) then
         if (AreaA[1,i]>0.999) then
            SCrB[i]:=SCrA[i]
         else
            SCrB[i]:=(AreaB[1,i]*ScrB[i]+Sum*SCrU[i])/(1-AreaA[1,i]);
      if (RotationType[i]=3) then
         if (AreaB[1,i]>0.999) then
            SCrA[i]:=SCrB[i]
        else
            SCrA[i]:=(AreaA[1,i]*ScrA[i]+Sum*SCrU[i])/(1-AreaB[1,i]);
      if RotationType[i]=4 then
         SCrA[i]:=AreaA[1,i]*SCrA[i]+AreaB[1,i]*SCrB[i]+Sum*SCrU[i];
    end;
  end; {for i:=1 to NrOfIntPoly do}
end; {with DataMod do}
...
//Reducing percolation losses from the canals when watertable is shallow
  ...
  IF DWT<0 then
  begin
    Byp[j,i]:=BYP[j,i]+PLCC+Rain[j,i]+IrrA[j,i]+IrrB[j,i];
    PLCC:=0;
    Rain[j,i]:=0;
    IrrA[j,i]:=0;
    IrrB[j,i]:=0;
