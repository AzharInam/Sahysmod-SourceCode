...
interface
  ...
  var Qaq, Byp, EpU0, AreaU, AA0, BB0,
      IRA0, IRB0, EpA0, EpB0, CIR, CIT     : array of array of real;
      SALY, SurfConc, StorWL, AvSal         : array of real;
      ...
      AllWarning, NoIrr, Watch              : boolean;
      RotaChanged, ExcessNoted              : boolean;
      InitWarning, RotaWarning   : boolean;
      ExcessWarning              : boolean;
      ReUseWarning, IrrWarning   : boolean;
      AreaWarning, WellWarning   : boolean;
...
//in unit UMainCalc
  setlength (CIT,NrOfSeasons+1,NrOfIntPoly+1);
  setlength (CIR,NrOfSeasons+1,NrOfIntPoly+1);
...
//Total amount of canal irrigation water including re-use
  ...
  begin
    ...
    CIT[j,i]:=PLCC+IrrA[j,i]*AreaA[j,i]+IrrB[j,i]*AreaB[j,i]
         +ReUseDr[j,i]+ReUseWell[j,i]*WellDis[j,i];
    CIR[j,i]:=CIT[j,i]-PLCC-ReUseDr[j,i]-ReUseWell[j,i]*WellDis[j,i];

//Canal water excluding re-use
    if AreaU[j,i]<1 then
    begin
      IrrA[j,i]:=IrrA[j,i]+ReUseDr[j,i]*AreaA[j,i]/(1-AreaU[j,i]);
      IrrB[j,i]:=IrrB[j,i]+ReUseDr[j,i]*AreaB[j,i]/(1-AreaU[j,i]);
      IrrA[j,i]:=IrrA[j,i]+ReUseWell[j,i]*WellDis[j,i]*AreaA[j,i]/(1-AreaU[j,i]);
      IrrB[j,i]:=IrrB[j,i]+ReUseWell[j,i]*WellDis[j,i]*AreaB[j,i]/(1-AreaU[j,i]);
    end else
    begin
      IrrA[j,i]:=IrrA[j,i]+ReUseDr[j,i]*AreaA[j,i];
      IrrB[j,i]:=IrrB[j,i]+ReUseDr[j,i]*AreaB[j,i];
      IrrA[j,i]:=IrrA[j,i]+ReUseWell[j,i]*WellDis[j,i]*AreaA[j,i];
      IrrB[j,i]:=IrrB[j,i]+ReUseWell[j,i]*WellDis[j,i]*AreaB[j,i];
    end;
  end;
...
//Changing safe rotation type if possible when given rotation type is 0
  for i:=1 to NrOfIntPoly do if InitRota[i]=0 then
  begin
    if ACarea[i]+BCArea[i]+UCArea[i]>0.999 then
          RotationType[i]:=0;
    if ACarea[i]+BCArea[i]+UCArea[i]<1 then
    begin
      if (ACarea[i]>0) and (BCarea[i]>0) and (UCarea[i]>0) then
         RotationType[i]:=10;
      if (ACarea[i]>0) and (BCarea[i]>0) and (UCarea[i]<0.001) then
          RotationType[i]:=5;
      if (ACarea[i]>0) and (UCarea[i]>0) and (BCarea[i]<0.001) then
          RotationType[i]:=6;
      if (BCarea[i]>0) and (UCarea[i]>0) and (ACarea[i]<0.001) then
          RotationType[i]:=7;
      if (ACarea[i]>0) and (BCarea[i]<0.001) and (UCarea[i]<0.001) then
          RotationType[i]:=2;
      if (BCarea[i]>0) and (ACarea[i]<0.001) and (UCarea[i]<0.001) then
          RotationType[i]:=3;
      if (UCarea[i]>0) and (ACarea[i]<0.001) and (BCarea[i]<0.001) then
          RotationType[i]:=1;
    end;
  end;

//Changing safe rotation type if possible when given rotation type is 5
  for i:=1 to NrOfIntPoly do if InitRota[i]=5 then
  begin
    if (ACarea[i]>0) and (BCarea[i]>0) then RotationType[i]:=5;
    if (ACarea[i]>0) and (BCarea[i]<0.001) then RotationType[i]:=2;
  end;
//Changing safe rotation type if possible when given rotation type is 6
  for i:=1 to NrOfIntPoly do if InitRota[i]=6 then
  begin
    if (ACarea[i]>0) and (UCarea[i]>0) then RotationType[i]:=6;
    if (ACarea[i]>0) and (UCarea[i]<0.001) then RotationType[i]:=2;
  end;
//Changing safe rotation type if possible when given rotation type is 7
  for i:=1 to NrOfIntPoly do if InitRota[i]=7 then
  begin
    if (BCarea[i]>0) and (UCarea[i]>0) then RotationType[i]:=7;
    if (BCarea[i]>0) and (UCarea[i]<0.001) then RotationType[i]:=3;
  end;
  for i:=1 to NrOfIntPoly do if InitRota[i]=10 then
  begin
    if (BCarea[i]>0) and (ACarea[i]>0) and (UCarea[i]>0) then
        RotationType[i]:=10;
    if (ACarea[i]>0) and (BCarea[i]>0) and (UCarea[i]<0.001) then
    begin
      if ACarea[i]>=BCarea[i] then RotationType[i]:=2;
      if ACarea[i]<BCarea[i] then RotationType[i]:=3;
    end;
    if (ACarea[i]>0) and (BCarea[i]<0.001) and (UCarea[i]>0) then
        RotationType[i]:=1;
    if (ACarea[i]<0.001) and (BCarea[i]>0) and (UCarea[i]>0) then
        RotationType[i]:=3;
    if (ACarea[i]>0) and (BCarea[i]>0) and (UCarea[i]<0.001) then
        RotationType[i]:=2;
    if ACarea[i]+BCArea[i]+UCArea[i]<0.001 then
       RotationType[i]:=4;
(*
    if ((ACarea[i]<0.001) and (BCarea[i]<0.001)) or
       ((ACArea[i]<0.001) and (UCarea[i]<0.001)) or
       ((BCarea[i]<0.001) and (UCarea[i]<0.001)) then
       RotationType[i]:=0;
*)
  end;
...
*)
  For i:=1 to NrOfIntPoly do
  begin
//Change when area reductions were made due to salinity
    if Watch and (UCarea[i]>0.001) then
    begin
      if InitRota[i]<5 then RotationType[i]:=1;
      if InitRota[i]=3 then RotationType[i]:=3;
      if (InitRota[i]=5) or (InitRota[i]=6) then RotationType[i]:=6;
      if InitRota[i]=7 then RotationType[i]:=7;
    end;
//Restoring rotation type when area reductions have been undone
    if (InitRota[i]<>1) and (RotationType[i]=1) and (UCarea[i]<0.001) then
    begin
      Watch:=false;
      RotationType[i]:=InitRota[i];
    end;
    if (InitRota[i]<>6) and (RotationType[i]=6) and (UCarea[i]<0.001) then
    begin
      Watch:=false;
      RotationType[i]:=InitRota[i];
    end;
    if (InitRota[i]<>7) and (RotationType[i]=7) and (UCarea[i]<0.001) then
    begin
      Watch:=false;
      RotationType[i]:=InitRota[i];
    end;
//Restoring false settings, probably not necessary
    if (InitRota[i]<>2) and (RotationType[i]=2) and (ACarea[i]<0.001) then
        RotationType[i]:=InitRota[i];
    if (InitRota[i]<>3) and (RotationType[i]=3) and (BCarea[i]<0.001) then
        RotationType[i]:=InitRota[i];
    if (InitRota[i]<>5) and (RotationType[i]=5)
        and (ACarea[i]+BCarea[i]<0.001) then
             RotationType[i]:=InitRota[i];
  end; {for i:=1 to NrOfIntPoly do }


  for i:=1 to NrOfIntPoly do
  if not RotaWarning and (RotationType[i]<>InitRota[i]) then
  begin
    RotaChanged:=true;
    AllWarning:=true;
    HlpTxt1 := inttostr (i);
...
*)
//Adjusting salinity settings (een beetje willekeurig maar komt weinig voor)
  if not RotaPassed then for i:=1 to NrOfIntPoly do
  begin
    Uarea:=1-AreaA[1,i]-AreaB[1,i];
    if (RotationType[i]=0) or (RotationType[i]=10) then
        if (ACarea[i]>=BCarea[i]) and (Uarea<0.001) then SCrU[i]:=SCrA[i];
    if (RotationType[i]=0) or (RotationType[i]=10) then
        if (ACarea[i]<BCarea[i]) and (Uarea<0.001) then SCrU[i]:=SCrB[i];
    if (RotationType[i]=1) or (RotationType[i]=6) or (RotationType[i]=7) then
        if (ACarea[i]>=BCarea[i]) and (Uarea<0.001) then SCrU[i]:=SCrA[i];
    if (RotationType[i]=1) or (RotationType[i]=6) or (RotationType[i]=7) then
        if (ACarea[i]<BCarea[i]) and (Uarea<0.001) then SCrU[i]:=SCrB[i];
  end;
...
procedure InitSalinity;                              {part of AdjustRotation}
var i : integer;
    DA, DB, DU, Uarea : real;
...
//Annual claculations without famers' responses are not treated. Where else ?

  for i:=1 to NrOfIntPoly do
  begin
    if (RotationType[i]=1) and (SCrU[i]<=0) then SCrU[i]:=SCrA[i];
    if (RotationType[i]=2) and (SCrA[i]<=0) then SCrA[i]:=SCrB[i];
    if (RotationType[i]=3) and (SCrB[i]<=0) then SCrB[i]:=SCrA[i];
    if (RotationType[i]=5) and (SCrA[i]<=0) then SCrA[i]:=SCrU[i];
    if (RotationType[i]=5) and (SCrB[i]<=0) then SCrB[i]:=SCrU[i];
    if (RotationType[i]=6) and (SCrA[i]<=0) then SCrA[i]:=SCrB[i];
    if (RotationType[i]=6) and (SCrU[i]<=0) then SCrU[i]:=SCrB[i];
    if (RotationType[i]=7) and (SCrB[i]<=0) then SCrB[i]:=SCrA[i];
    if (RotationType[i]=7) and (SCrU[i]<=0) then SCrU[i]:=SCrA[i];
//What is the use of the above ??

//The following only in the 1st year, even with annual calculations,
//but no farmers' responses
    Uarea:=1-AreaA[1,i]-AreaB[1,i];
    if (YearNr<1) and (ResponsIndex[i]=0) then
    begin
      if (RotationType[i]=1) then    {fixed U area}
         if (Uarea>0.999) then
            SCrA[i]:=SCrU[i]
         else if Uarea<1 then
            SCrA[i]:=(AreaA[1,i]*ScrA[i]+AreaB[1,i]*SCrB[i])/(1-Uarea);
      if (RotationType[i]=2) then                                {fixed A area}
         if (AreaA[1,i]>0.999) then
            SCrB[i]:=SCrA[i]
         else if AreaA[1,i]<1 then
            SCrB[i]:=(AreaB[1,i]*ScrB[i]+Uarea*SCrU[i])/(1-AreaA[1,i]);
      if (RotationType[i]=3) then                                {fixed B area}
         if (AreaB[1,i]>0.999) then
            SCrA[i]:=SCrB[i]
        else if AreaB[1,i]<1 then
            SCrA[i]:=(AreaA[1,i]*ScrA[i]+Uarea*SCrU[i])/(1-AreaB[1,i]);
      if RotationType[i]=4 then                 {full rotation, no fixed areas}
         SCrA[i]:=AreaA[1,i]*SCrA[i]+AreaB[1,i]*SCrB[i]+Uarea*SCrU[i];
      if RotationType[i]=5 then                           {fixed A and B areas}
      begin
        if (AreaU[1,i]>0.999) then
        begin
          SCrA[i]:=SCrU[i];
          SCrB[i]:=SCrU[i];
        end else
        begin
          if AreaA[1,i]+Uarea>0 then
             SCrA[i]:=(AreaA[1,i]*ScrA[i]+Uarea*SCrU[i])/(AreaA[1,i]+Uarea);
          if AreaB[1,i]+Uarea>0 then
             SCrB[i]:=(AreaB[1,i]*ScrB[i]+Uarea*SCrU[i])/(AreaB[1,i]+Uarea);
//          SCRZ[i]:=(AreaA[1,i]*ScrA[i]+AreaB[1,i]*ScrB[i]/
//                   (AreaA[1,i]+AreaB[1,i]);
        end;
      end;
      if RotationType[i]=6 then                           {fixed A and U areas}
      begin
        if (AreaB[1,i]>0.999) then
        begin
          SCrA[i]:=SCrB[i];
        ...
        begin
          if AreaA[1,i]+Uarea>0 then
             SCrA[i]:=(AreaA[1,i]*ScrA[i]+Uarea*SCrB[i])/(AreaA[1,i]+Uarea);
          if AreaB[1,i]+Uarea>0 then
             SCrU[i]:=(Uarea*ScrU[i]+Uarea*SCrB[i])/(Uarea+Uarea);
//          SCRZ[i]:=(AreaA[1,i]*ScrA[i]+AreaU[1,i]*ScrU[i]/
//                   (AreaA[1,i]+AreaU[1,i]);
        end;
      end;
      if RotationType[i]=7 then                           {fixed B and U areas}
      begin
        if (AreaB[1,i]>0.999) then
        begin
          SCrA[i]:=SCrA[i];
        ...
        begin
          if AreaB[1,i]+Uarea>0 then
             SCrB[i]:=(AreaB[1,i]*ScrB[i]+Uarea*SCrA[i])/(AreaB[1,i]+Uarea);
          if Uarea+Uarea>0 then
             SCrU[i]:=(Uarea*ScrU[i]+Uarea*SCrA[i])/(Uarea+Uarea);
//          SCRZ[i]:=(AreaU[1,i]*ScrU[i]+AreaB[1,i]*ScrB[i]/
//                   (AreaU[1,i]+AreaB[1,i]);
        end;
      end;
//      if RotationType[i]=10 then
//         SCrZ[i]:=(AreaA[1,i]*SCRA[i]+AreaB[1,i]*SCRB[i]+AreaU[1,i]*SCRU[i])
//         /(AreaA[1,i]+AreaB[1,i]+AreaU[1,i]);
    end;
  end; {for i:=1 to NrOfIntPoly do}

//The following in the first year only when Farmers' responses are on
//not for annual calculations because then reponses are the turned off
  ...
  begin
    ...
    if RotationType[i]=6 then            {fixed A en U, rest B}
    begin
      if DU>0 then
         SCrU[i]:=(UCParea[i]*SCrU[i]+DB*SCrB[i])/(UCParea[i]+DB);
      if DA>0 then
         SCrA[i]:=(ACParea[i]*SCrA[i]+DA*SCrB[i])/(ACParea[i]+DA);
      if DB>0 then
         SCrB[i]:=(BCParea[i]*SCrB[i]+0.5*DB*(SCrA[i]+SCRU[i]))/(BCParea[i]+DB);
    end;
    if RotationType[i]=7 then          {fixed B en U, rest A}
    begin
      if DB>0 then
         SCrB[i]:=(BCParea[i]*SCrB[i]+DB*SCrA[i])/(BCParea[i]+DB);
      if DU>0 then
         SCrU[i]:=(UCParea[i]*SCrU[i]+DU*SCrA[i])/(UCParea[i]+DU);
      if DA>0 then
         SCrA[i]:=(ACParea[i]*SCrA[i]+0.5*DA*(SCRB[i]+SCrU[i]))/(ACParea[i]+DA);
    end;
//Check in which proportion A and B areas are abandoned
  end; {for i:=1 to NrOfIntPoly do if ResponsIndex[i]=1 then}

//Intial values of SCRZ
  for i:=1 to NrOfIntPoly do
  begin
    if RotationType[i]>4 then
    begin
      if AreaA[1,i]+AreaB[1,i]+AreaU[1,i]>0 then
         SCrZ[i]:=(AreaA[1,i]*SCRA[i]+AreaB[1,i]*SCRB[i]+AreaU[1,i]*SCRU[i])
         /(AreaA[1,i]+AreaB[1,i]+AreaU[1,i])
      else SCRZ[i]:=0;
    end;
...
//Percolation losses in irrigated A fields
  ...
  PLA:=Rain[j,i]+IrrA[j,i]-EvapA[j,i]-SurfDrA[j,i];
...
//Percolation losses in irrigated B fields
  PLB:=Rain[j,i]+IrrB[j,i]-EvapB[j,i]-SurfDrB[j,i];
...
//Percolation losses in unirrigated U fields
  PLU:=Rain[j,i]+SurfInflow[j,i]-EvapU[j,i]-SurfOutFlow[j,i];
...
//Total irrigation efficiency
    EFT:=0;
    if CIT[j,i]>0.001 then
       EFT:=((EvapA[j,i]-CAA)*AreaA[j,i]+(EvapB[j,i]-CAB)*AreaB[j,i])
            /(CIT[j,i]+Rain[j,i]);
    if EFT>1 then EFT:=1;
...
//Irrigation output
  ...
  if AreaA[j,i]<0.001 then AreaA[j,i]:=0;
  if AreaB[j,i]<0.001 then AreaB[j,i]:=0;
  AIR:=IrrA[j,i];
  BIR:=IrrB[j,i];
  if AIR+BIR<0.0002 then CIT[j,i]:=0;
  if AIR+BIR<0.0002 then CIR[j,i]:=0;

  if (AreaA[j,i]>0) and (AreaB[j,i]>0) then
     writeln (OutputFile,'  It  = ',format('%10.3e',[CIT[j,i]]),'    Is  = ',
               format('%10.3e',[CIR[j,i]]),'    IaA = ',format('%10.3e',[AIR]),
               '    IaB = ',format('%10.3e',[BIR]));
  if (AreaA[j,i]>0) and (AreaB[j,i]<0.00001) then
      Writeln (OutputFile,'  It  = ',format('%10.3e',[CIT[j,i]]),'    Is  = ',
               format('%10.3e',[CIR[j,i]]),'    IaA = ',format('%10.3e',[AIR]),
               '    IaB = ','     -    ');
  if (AreaA[j,i]<0.00001) and (AreaB[j,i]>0) then
      Writeln (OutputFile,'  It  = ',format('%10.3e',[CIT[j,i]]),'    Is  = ',
               format('%10.3e',[CIR[j,i]]),'    IaA = ','     -    ',
               '    IaB = ',format('%10.3e',[BIR]));
  if (AreaA[j,i]<0.00001) and (AreaB[j,i]<0.00001) then
...
//Efficiencies and sufficiencies
  ...
  if (AreaA[j,i]>0) and (AreaB[j,i]>0) then
  begin
    if (EvapU[j,i]>0) and (EvapU[j,i]<0.001) then EvapU[j,i]:=0;
...
//Percolation and Capillary rise
  ...
  begin
    ...
    if (AreaA[j,i]<0.00001) and (AreaB[j,i]>0.001) then
    begin
...
//Average salinity per polygon

  if (RotationType[i]=0) or (RotationType[i]=10) then
     AvSal[i]:=ACarea[i]*SCrA[i]+BCarea[i]*ScrB[i]+UCarea[i]*ScrU[i]+
               (1-ACarea[i]-BCArea[i]-UCarea[i])*ScrZ[i];
  if (RotationType[i]=1) then
     AvSal[i]:=UCarea[i]*SCrU[i]+(1-UCarea[i])*SCrA[i];
  if (RotationType[i]=2) then
     AvSal[i]:=ACarea[i]*SCrA[i]+(1-ACarea[i])*SCrB[i];
  if (RotationType[i]=3) then
     AvSal[i]:=BCarea[i]*SCrB[i]+(1-BCarea[i])*SCrA[i];
  if (RotationType[i]=4) then
     AvSal[i]:=SCrA[i];
  if (RotationType[i]=5) then
     AvSal[i]:=ACarea[i]*SCrA[i]+BCarea[i]*SCrB[1]+
     (1-ACarea[i]-BCarea[i])*SCrZ[i];
  if (RotationType[i]=6) then
     AvSal[i]:=ACarea[i]*SCrA[i]+UCarea[i]*SCrU[1]+
     (1-ACarea[i]-UCarea[i])*SCrZ[i];
  if (RotationType[i]=7) then
     AvSal[i]:=BCarea[i]*SCrB[i]+UCarea[i]*SCrU[1]+
...
//Areas
  ...
  if (AreaA[j,i]>0) and (AreaB[j,i]>0) and (AreaU[j,i]<0.00001) then
  writeln (OutputFile,'  A   = ',format('%10.3e',[AreaA[j,i]]),
  '    B   = ',format('%10.3e',[AreaB[j,i]]),'    U   = ','     -    ');
  if (AreaA[j,i]>0) and (AreaB[j,i]<0.00001) and (AreaU[j,i]>0) then
  writeln (OutputFile,'  A   = ',format('%10.3e',[AreaA[j,i]]),
  '    B   = ','     -    ','    U   = ',format('%10.3e',[AreaU[j,i]]));
  if (AreaA[j,i]>0) and (AreaB[j,i]<0.00001) and (AreaU[j,i]<0.00001) then
  writeln (OutputFile,'  A   = ',format('%10.3e',[AreaA[j,i]]),
  '    B   = ','     -    ','    U   = ','     -    ');
  if (AreaA[j,i]<0.00001) and (AreaB[j,i]>0) and (AreaU[j,i]>0) then
  writeln (OutputFile,'  A   = ','     -    ',
  '    B   = ',format('%10.3e',[AreaB[j,i]]),
  '    U   = ',format('%10.3e',[AreaU[j,i]]));
  if (AreaA[j,i]<0.00001) and (AreaB[j,i]>0) and (AreaU[j,i]<0.00001) then
  writeln (OutputFile,'  A   = ','     -    ',
  '    B   = ',format('%10.3e',[AreaB[j,i]]),
  '    U   = ','     -    ');
  if (AreaA[j,i]<0.00001) and (AreaB[j,i]<0.00001) and (AreaU[j,i]>0) then
  writeln (OutputFile,'  A   = ','     -    ',
  '    B   = ','     -    ', '    U   = ',format('%10.3e',[AreaU[j,i]]));
...
//Soil salinities in rootzone, depending on rotationtype
  IF (RotationType[i]=0) OR (RotationType[i]=10) THEN
  begin
    if AreaA[j,i]<0.00001 then SCrA[i]:=0;
    if AreaB[j,i]<0.00001 then SCrB[i]:=0;
    if AreaU[j,i]<0.00001 then SCrU[i]:=0;
    IF (ACarea[i]>0.00001) AND (BCarea[i]>0.001) AND (UCarea[i]>0.001) THEN
    ...
    IF (ACarea[i]<0.00001) AND (BCarea[i]>0.001) AND (UCarea[i]>0.001) THEN
        ...
        format('%10.3e',[SCrU[i]]),
                 '    Cr4 = ','     -    ');
    IF (ACarea[i]>0.00001) AND (BCarea[i]<0.00001) AND (UCarea[i]>0.001) THEN
        Writeln (OutputFile,'  CrA = ',format('%10.3e',[SCrA[i]]),
        '    CrB = ','     -    ','    CrU = ',format('%10.3e',[SCrU[i]]),
        '    Cr4 = ','     -    ');
    IF (ACarea[i]>0.001) AND (BCarea[i]>0.001) AND (UCarea[i]<0.00001) THEN
        Writeln (OutputFile,'  CrA = ',format('%10.3e',[SCrA[i]]),
        '    CrB = ',format('%10.3e',[SCrB[i]]),'    CrU = ','     -    ',
        '    Cr4 = ','     -    ');
    IF (ACarea[i]>0.001) AND (BCarea[i]<0.00001) AND (UCarea[i]<0.001) THEN
        Writeln (OutputFile,'  CrA = ',format('%10.3e',[SCrA[i]]),
        '    CrB = ','     -    ','    CrU = ','     -    ',
        '    Cr4 = ','     -    ');
    IF (ACarea[i]<0.00001) AND (BCarea[i]>0.001) AND (UCarea[i]<0.00001) THEN
        Writeln (OutputFile,'  CrA = ','     -    ','    CrB = ',
        format('%10.3e',[SCrB[i]]),'    CrU = ','     -    ',
        '    Cr4 = ','     -    ');
    IF (ACarea[i]<0.00001) AND (BCarea[i]<0.00001) AND (UCarea[i]>0.001) THEN
      ...
      if ACarea[i]+BCarea[i]+UCarea[i]>0.999 then
        Writeln (OutputFile,'  C0* = ','     -    ',
        '    C1* = ','     -    ','    C2* = ','     -    ',
        '    C3* = ','     -    ')
      else
        Writeln (OutputFile,'  C0* = ',format('%10.3e',[SCrZ[i]]),
        '    C1* = ','     -    ','    C2* = ','     -    ',
        '    C3* = ','     -    ');
  end; {IF (RotationType[i]=0) OR (RotationType[i]:=10) THEN}
  ...
  begin
    ...
    '    CrB = ',format('%10.3e',[SCrB[i]]),'    CrU = ','     -    ',
    '    Cr4 = ','     -    ');
    if ACArea[i]+BCArea[i]<0.999 then
       writeln (OutputFile,'  C0* = ','     -    ',
       '    C1* = ',format('%10.3e',[SCrZ[i]]),
       '    C2* = ','     -    ','    C3* = ','     -    ')
    else
       writeln (OutputFile,'  C0* = ','     -    ',
       '    C1* = ','     -    ','    C2* = ',
  ...
  begin
    ...
    '    CrB = ','     -    ','    CrU = ',format('%10.3e',[SCrU[i]]),
    '    Cr4 = ','     -    ');
    if ACArea[i]+UCArea[i]<0.999 then
       writeln (OutputFile,'  C0* = ','     -    ',
       '    C1* = ',format('%10.3e',[SCrZ[i]]),
       '    C2* = ','     -    ','    C3* = ','     -    ')
    else
  ...
  if RotationType[i]=7 then
  begin
    writeln (OutputFile,'  CrA = ','     -    ',
    '    CrB = ',format('%10.3e',[SCrB[i]]),
    '    CrU = ',format('%10.3e',[SCrU[i]]),
    '    Cr4 = ','     -    ');
    if ACArea[i]+UCArea[i]<0.999 then
       writeln (OutputFile,'  C0* = ','     -    ',
