...
begin
  ...
  for j:=1 to DataMod.NrOfSeasons do for k:=0 to DataMod.NrOfIntPoly-1 do
  begin
    if DataMod.AuxValue[1,j,k]+DataMod.AuxValue[2,j,k]>1 then
    begin
...
begin
  ...
  begin
    ...
    begin
      if (InitAreaA[j,k]<=0) and (AreaA[j,k]>0) then
          MainForm.AreaChange:=true;
      if (InitAreaB[j,k]<=0) and (AreaB[j,k]>0) then
          MainForm.AreaChange:=true;
      if (InitAreaA[j,k]+InitAreaB[j,k]=1) and ((AreaA[j,k]+AreaB[j,k])<1)
          then MainForm.AreaChange:=true;
    end;
    for j:=1 to NrOfSeasons do for k:=0 to NrOfIntPoly-1 do
    begin
      InitAreaA[j,k]:=AreaA[j,k];
      InitAreaB[j,k]:=AreaB[j,k];
    end;
  end;
end; {CompareAreaData}
{--------------------}
